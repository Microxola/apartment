...
const SearchResult = () => {
  ...
  const urlTransactionType = queryParams.get('transactionType');
  const urlPropertyType = queryParams.get('propertyType');
  const urlLocation = queryParams.get('location');
  const urlPriceRange = queryParams.get('priceRange');
  
  // State for filters - initialize with URL parameters
  const [filters, setFilters] = useState({
    transactionType: urlTransactionType || '',
  ...
  useEffect(() => {
    ...
    // Apply transaction type filter (buy/rent/sell)
    if (filters.transactionType) {
      result = result.filter(room => room.Type.toLowerCase() === filters.transactionType.toLowerCase());
    }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold">
            {filters.transactionType ? `Properties for ${filters.transactionType}` : 'All Properties'}
            {filters.location && ` in ${filters.location}`}
          </h2>
          {filteredRooms.length > 0 && (
            <p className="text-gray-600">
              Found {filteredRooms.length} properties matching your criteria
            </p>
          )}
        </div>
